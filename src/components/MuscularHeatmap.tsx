...
const STYLES: Record<
  IntensityLevel,
  { light: string; dark: string; stroke: string; sw: number; opacity: number }
> = {
  0: { light: "#6a6a72", dark: "#50505a", stroke: "#3a3a44", sw: 0.25, opacity: 0.3 },
  1: { light: "#8a4848", dark: "#6a3030", stroke: "#5a2828", sw: 0.4, opacity: 1 },
  2: { light: "#d04040", dark: "#a02828", stroke: "#801818", sw: 0.45, opacity: 1 },
  3: { light: "#ff4040", dark: "#c01515", stroke: "#ff3030", sw: 0.55, opacity: 1 },
};

const LEGEND_COLORS: Record<IntensityLevel, { fill: string; border: string }> = {
  0: { fill: "#5a5a62", border: "#70707a" },
  1: { fill: "#7a3838", border: "#8a4848" },
  2: { fill: "#c03535", border: "#dd4545" },
  3: { fill: "#e02020", border: "#ff4040" },
};
...
function BodyView({
  ...
  return (
    <svg viewBox="16 6 68 132" className="w-[120px] sm:w-[150px] h-auto select-none">
      <defs>
        {/* Body base gradient */}
        <linearGradient id={`${vid}-body`} x1="0" y1="0" x2="0.15" y2="1">
          <stop offset="0%" stopColor="#6e6e76" />
          <stop offset="100%" stopColor="#4a4a52" />
        </linearGradient>

        {/* Per-level radial gradients for 3D volume */}
        {([0, 1, 2, 3] as IntensityLevel[]).map((lvl) => {
          const s = STYLES[lvl];
          return (
            <radialGradient key={lvl} id={`${vid}-g${lvl}`} cx="35%" cy="28%" r="68%">
        ...
        {/* Drop-shadow filter for raised 3D muscle surfaces */}
        <filter id={`${vid}-vol`} x="-8%" y="-8%" width="120%" height="120%">
          <feGaussianBlur in="SourceAlpha" stdDeviation="0.7" result="sh" />
          <feOffset in="sh" dx="0.4" dy="0.7" result="osh" />
          <feFlood floodColor="#000" floodOpacity="0.35" result="sf" />
          <feComposite in="sf" in2="osh" operator="in" result="shadow" />
          <feMerge>
        ...
        {/* Neon glow filter for level 3 (intense) */}
        <filter id={`${vid}-glow`} x="-30%" y="-30%" width="160%" height="160%">
    ...
    <div className="card !p-0 overflow-hidden">
          ...
          {([0, 1, 2, 3] as IntensityLevel[]).map((lvl) => (
            <span key={lvl} className="flex items-center gap-1 text-[10px] text-text-dim">
              <span
                ...
                style={{
                  background: LEGEND_COLORS[lvl].fill,
                  border: `1px solid ${LEGEND_COLORS[lvl].border}`,
                }}
              />
              {LEVEL_LABEL[lvl]}
            </span>
      ...
      <div className="flex justify-center gap-2 px-3 py-3 bg-[rgb(var(--c-bg))]/60">
        <div className="text-center">
          <BodyView vid="f" zones={F} scores={scores} maxScore={maxScore} />
          <div className="text-[10px] text-text-dim mt-1 font-medium tracking-wide">FACE</div>
        </div>
        <div className="text-center">
          <BodyView vid="b" zones={B} scores={scores} maxScore={maxScore} />
